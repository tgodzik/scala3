  ...
  abstract class Application[Arg](methRef: TermRef, funType: Type, args: List[Arg], resultType: Type)(using Context) {
    ...
    def reorder[T <: Untyped](args: List[Trees.Tree[T]]): List[Trees.Tree[T]] = {
      ...
      def handleNamed(pnames: List[Name], args: TreeList[T],
                      nameToArg: Map[Name, Trees.NamedArg[T]], toDrop: Set[Name]): TreeList[T] = pnames match {
        case pname :: pnames if nameToArg.contains(pname) =>
          ...
          arg :: handleNamed(pnames, args, nameToArg - pname, toDrop + pname)
        ...
        case _ =>
          ...
          case allArgs @ (arg @ NamedArg(aname, _)) :: args =>
            if toDrop.contains(aname) then
              ...
              genericEmptyTree :: handleNamed(pnames.tail, allArgs, nameToArg, toDrop)
              ...
              arg :: handleNamed(pnames.dropOne, args, nameToArg, toDrop)
          case arg :: args =>
            if toDrop.nonEmpty then
              report.error(i"positional after named argument", arg.srcPos)
            arg :: handleNamed(pnames.dropOne, args, nameToArg, toDrop) // unnamed argument; pick it
          case nil => // no more args, continue to pick up any preceding named args
            if pnames.isEmpty then nil
            else handleNamed(pnames.dropOne, args = nil, nameToArg, toDrop)
      }
      ...
      def handlePositional(pnames: List[Name], args: TreeList[T]): TreeList[T] =
        ...
        case (_: NamedArg) :: _ =>
          ...
          handleNamed(pnames, args, nameAssocs.toMap, toDrop = Set.empty)
        case arg :: args =>
