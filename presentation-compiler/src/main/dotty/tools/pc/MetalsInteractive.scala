...
object MetalsInteractive:
  ...
  /** If `pos` is on a type class name in a `derives` clause, return that type class
   *  symbol and type. Synthesized givens for derivations store the user-written type
   *  tree in [[OriginalTypeClass]] (see `typer.Deriving`).
   */
  def derivingOriginalTypeClass(
      path: List[Tree],
      pos: SourcePosition
  ): Option[(Symbol, Type)] =
    path.collectFirst { case t: Tree if t.span.contains(pos.span) =>
      t.getAttachment(OriginalTypeClass).map { orig =>
        val sym = orig.symbol.sourceSymbol
        val tpe = orig.typeOpt.widen match
          case tp if tp.exists => tp
          case _               => sym.info
        (sym, tpe)
      }
    }.flatten
  ...
  def enclosingSymbolsWithExpressionType(
    ...
    import indexed.ctx
    derivingOriginalTypeClass(path, pos) match
      case Some((sym, tpe)) =>
        List((sym, tpe, None))
      case None =>
        path match
